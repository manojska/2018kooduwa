...
void colorConsume(CRGB new_color) {
  ...
  for (byte i =0; i < SIDE_LENGTH; i++) {
    ...
    for (byte j = i+1; j < SIDE_LENGTH; j++) {
      level_1[j].nscale8_video(192);
      level_1[25-j].nscale8_video(192);
      level_1[j+26].nscale8_video(192);
      level_1[51-j].nscale8_video(192);
      level_1[j+52].nscale8_video(192);
      level_1[78-j].nscale8_video(192);
    }
    FastLED.show(); delay(anime_delay);
    level_1[i] = new_color;
    level_1[25-i] = new_color;
    level_1[i+26] = new_color;
    level_1[51-i] = new_color;
    level_1[i+52] = new_color;
    level_1[78-i] = new_color;
    FastLED.show(); delay(anime_delay);
  }
}
